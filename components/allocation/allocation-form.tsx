...
import { CSV_HEADERS, SEGMENTS } from "@/types/allocation";
...
export function AllocationForm() {
  ...
  function handleDownload() {
    ...
    anchor.href = url;
    anchor.download = "allocation.csv";
    anchor.click();
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4 text-sm">
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
              <div className="space-y-2">
                <label className="text-xs font-medium text-muted-foreground">총 배분 대수</label>
                <input
                  type="number"
                  min={1}
                  className="h-11 w-full rounded-2xl border border-border/70 bg-background px-4 text-sm outline-none transition focus:border-foreground focus:ring-4 focus:ring-foreground/10"
        ...
        {result && !loading && (
          <div className="space-y-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h3 className="text-lg font-semibold tracking-[-0.03em] text-foreground">배분 결과</h3>
                <p className="text-sm text-muted-foreground">
