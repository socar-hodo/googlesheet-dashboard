...
export function buildCostBreakdownDaily(
  ...
  return rows.map((r) => ({
    ...
    washCost: safeNumber(r.wash_cost),
    maintenanceCost: safeNumber(r.maintenance_cost),
    repairCost: safeNumber(r.repair_cost),
    insuranceCost: safeNumber(r.insurance_cost),
    taxCost: safeNumber(r.tax_cost),
    communicationCost: safeNumber(r.communication_cost),
    chargeEvCost: safeNumber(r.charge_ev_cost),
    // 운반비 드릴다운(COST-02) — SQL에 미포함, 추후 _cost_transport_* 컬럼 연결 예정
...
// 순수 함수는 별도 파일에서 재수출 (server-only 없이 테스트 가능)
export {
  sanitizeRegionName,
  buildRegionFilter,
  buildRegionOptions,
